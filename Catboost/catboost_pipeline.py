...
class CatBoostClassifier_wrapper():
    ...
    def fit(self, X, y):
        ...
        self.model = CatBoostClassifier(iterations=50).fit(X, y, cat_features = cat_features, text_features = text_features)
        return self.model
...
def make_pipeline(X):
    catboost_preprocessing = ColumnTransformer(
        transformers=[
            ...
            ("cat", SimpleImputer(strategy="constant", fill_value=""), DataSelector.CatSelector),
